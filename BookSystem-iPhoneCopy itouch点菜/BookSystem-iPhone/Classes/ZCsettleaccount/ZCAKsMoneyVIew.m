...
- (id)initWithFrame:(CGRect)frame dicMoney:(NSDictionary *)dicMoney
{
    ...
    if (self) {
        ...
        if ([[dicMoney objectForKey:@"OPERATE"] isEqualToString:@"1009"]) {
            tfMoney.text = @"50";
        }else if ([[dicMoney objectForKey:@"OPERATE"] isEqualToString:@"1008"]){
            tfMoney.text = @"100";
        }else if ([[dicMoney objectForKey:@"OPERATE"] isEqualToString:@"10"]){
...
- (void)confirm{
    if ([tfMoney.text length]<=0){
        ...
        [delegate MoneyViewWithOptions:dic];
    }
}

- (void)cancel{
    [delegate MoneyViewWithOptions:nil];
}

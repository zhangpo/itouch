...
#import "ZCSettleViewController.h"
...
@interface ZCSettleViewController ()

@end

@implementation ZCSettleViewController
...
- (void)loadData{
    @autoreleasepool {
        ...
        NSMutableDictionary *dict = (NSMutableDictionary *)[dp pQuery_zc:dic];
        if ([[dict objectForKey:@"account"] count] > 0){
            self.aryInfo = [[dict objectForKey:@"account"] retain];
            [dict removeObjectForKey:@"account"];
            dicInfoResult = [[NSDictionary dictionaryWithDictionary:dict] retain];
            [tvSerach reloadData];
        }else{
            NSString *msg;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    ZCSettleCell *cell = (ZCSettleCell *)[tableView dequeueReusableCellWithIdentifier:identifier];
    if (!cell){
        cell = [[[ZCSettleCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier] autorelease];
    }
...
-(NSString *)addCount{
    ...
    for (NSDictionary *dic1 in ary) {
        ...
        total = [[dic1 objectForKey:@"total"] floatValue];
        price = [[dic1 objectForKey:@"price"] floatValue];

        allPrice = allPrice + price;
    }
    [sharedData sharedInstance].yingShouMoney = [dicInfoResult objectForKey:@"total"];//应收金额
    
    NSString *yingfu = [NSString stringWithFormat:@"%.2f",[self MoLing:[dicInfoResult objectForKey:@"total"]]];
...
-(float)MoLing:(NSString *)money{
    ...
    //添加优惠服务费显示
    NSString *p = [dicInfoResult objectForKey:@"people"];
    NSArray *ary = [p componentsSeparatedByString:@"人"];
    if ([ary count] > 1) {
        if (![[ary objectAtIndex:1] isEqualToString:@" "] && [ary objectAtIndex:1] != NULL) {
            BOOL flagYH = YES;
            for (NSDictionary *dic in aryFangShi) {
            ...
            if (flagYH) {
                NSMutableDictionary *dicYouHui = [NSMutableDictionary dictionary];
                [dicYouHui setValue:@"优惠和服务费" forKey:@"OPERATENAME"];
                [dicYouHui setValue:[ary objectAtIndex:1] forKey:@"money"];
                [aryFangShi addObject:dicYouHui];
            }
        }
    }
    ...
    switch (item.tag) {
        ...
        case 0:{//   现金
            ...
            if([aryMoney count]==0)
            {
                ...
                {
                    [self dismissViews];
                    _moneyView=[[ZCAKsMoneyVIew alloc] init];
        ...
        case 1:{//   银行卡
            ...
            if([aryBank count]==0)
            {
                ...
                {
                    [self dismissViews];
                    bankCardView=[[ZCBankCardView alloc] init];
        ...
        case 2:{//   优惠方式
            ZCfavorableViewController *f = [[ZCfavorableViewController alloc] init];
...
-(void)ButtonClickMoney:(UIButton *)button
{
    ...
    _moneyView = [[ZCAKsMoneyVIew alloc] initWithFrame:CGRectMake(0, 0, 275, 200) dicMoney:dic];
...
-(void)ButtonClickBank:(UIButton *)button
{
    ...
    bankCardView = [[ZCBankCardView alloc] initWithFrame:CGRectMake(0, 0, 275, 200) dicMoney:dic];
    bankCardView.delegate = self;
...
-(void)addPrice{
    ...
    float yingshou = [[[sharedData sharedInstance] yingShouMoney] floatValue];

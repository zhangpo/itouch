...
@interface BSPackItemViewController ()

@end

@implementation BSPackItemViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([[itemDic objectForKey:@"defualtS"] isEqualToString:@"0"]) {
        cell.textLabel.textColor = [UIColor redColor];
    }else{
        cell.textLabel.textColor = [UIColor blackColor];
    }
    cell.textLabel.text = [itemDic objectForKey:@"PNAME"];
    NSString *price = [NSString stringWithFormat:@"%@/%@   ",[itemDic objectForKey:@"PRICE1"],[itemDic objectForKey:@"UNIT"]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    itemDic = [ary objectAtIndex:indexPath.row];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"ChangePackageItemNotification" object:itemDic];

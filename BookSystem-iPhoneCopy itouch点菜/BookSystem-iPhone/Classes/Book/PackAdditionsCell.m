...
@implementation PackAdditionsCell
@synthesize delegate;
...
- (void)showInfo:(NSDictionary *)info{
    ...
    NSString *strPrice = [NSString stringWithFormat:@"%@/%@",[info objectForKey:@"PRICE1"],[info objectForKey:@"UNIT"]];
...
- (void)additionsClicked{
    BSAdditionsView *v = [BSAdditionsView additionsViewWithDelegate:self additions:[self.itemInfo objectForKey:@"addition"]];
    [v show];
}

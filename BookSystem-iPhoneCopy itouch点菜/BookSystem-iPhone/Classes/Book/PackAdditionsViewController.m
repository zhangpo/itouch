...
@interface PackAdditionsViewController ()

@end

@implementation PackAdditionsViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    PackAdditionsCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (cell == nil) {
        cell = [[[PackAdditionsCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:cellIdentifier] autorelease];
        cell.delegate = self;
    }
...
- (void)cellUpdated:(PackAdditionsCell *)cell{
    ...
    for (NSMutableDictionary *mutDic in mutAry) {
        if ([[mutDic objectForKey:@"PCODE1"] isEqual:[cell.itemInfo objectForKey:@"PCODE1"]]) {

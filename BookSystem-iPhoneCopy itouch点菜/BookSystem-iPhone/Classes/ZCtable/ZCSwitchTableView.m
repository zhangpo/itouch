...
@implementation ZCSwitchTableView
...
- (void)confirm{
    ...
    if ([tfOldTable.text length]<=0 || [tfNewTable.text length]<=0 ){
        ...
        [alert show];
        [alert release];
    }else if ([newTable isEqualToString:oldTable]){//当前台位与目标台位不能相同
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:[langSetting localizedString:@"Error"] message:[langSetting localizedString:@"The same as the target the same not the same"] delegate:nil cancelButtonTitle:[langSetting localizedString:@"OK"] otherButtonTitles:nil];
        [alert show];
        [alert release];
    }else{
